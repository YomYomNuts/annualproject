...
void VoronoiGenerator::GenerateVoronoi(ObjectManager & gom, const Generic_Object * baseGameObject, const vec3 & positionCenterRotation, const vec3 & positionLookAt, const vec3 & positionTranlate, float radius, float height, float scale, const int particles)
{
	...
	for (unsigned int i = 0; i < baseGameObject->listFaces->size(); ++i)
	{
		...
		vec3 * p0 = &baseGameObject->verticesList->at(edge1->indexVertex1);
		vec3 * p1 = &baseGameObject->verticesList->at(edge1->indexVertex2);
		vec3 * p2 = &baseGameObject->verticesList->at(edge2->indexVertex1);
		vec3 * p3 = &baseGameObject->verticesList->at(edge2->indexVertex2);
		vec3 size = *p0 + *p1;
		vec3 cross;
	...
	if(vl.start())
	{
		do
		{
			if(con.compute_cell(c, vl))
			{
				...
				{
					cell->verticesList->push_back(vec3(xx + *ptsp * 0.5, yy + ptsp[1] * 0.5, zz + ptsp[2] * 0.5));
				}
