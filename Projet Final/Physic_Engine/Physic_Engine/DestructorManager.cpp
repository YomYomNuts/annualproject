...
void DestructorManager::LaunchDestruction(ObjectManager & gom, Generic_Object * goImpact, vec3 positionImpact, vec3 positionLookAt)
{
	...
	std::vector<unsigned short> * listVerticesConvexPolygon = DelaunayGenerator::LaunchCircleGeneration(8.0f, 40, go->verticesList, go->listEdges, go->listFaces);
	ParaboloidSurfaceProjection::LaunchProjection(8.0f, 5.0f, go->verticesList, go->listEdges, go->listFaces);
	go->Scale(0.7f);
	//VoronoiGenerator::GenerateVoronoi(gom, go, positionImpact, positionLookAt, vec3(0.0f, -2.5f, 0.0f), 8.0f, 5.0f, 0.7f, 5);
	go->TranslateAndLookAtAndScale(positionImpact, positionLookAt, vec3(0.0f, -2.5f, 0.0f), 10.3f);
	go->CalculateIndexes();
