...
- (void) mouseDown:(NSEvent *)event
{
    ...
    esgiMouseFunc(ESGI_LEFT_BUTTON, ESGI_DOWN, (int)point.x, __height - (int)point.y);
}

- (void) mouseUp:(NSEvent *)event
{
    NSPoint point = [self convertPoint:[event locationInWindow] fromView:nil];
    esgiMouseFunc(ESGI_LEFT_BUTTON, ESGI_UP, (int)point.x, __height - (int)point.y);
}

- (void) rightMouseDown:(NSEvent *)event
{
    NSPoint point = [self convertPoint:[event locationInWindow] fromView:nil];
    esgiMouseFunc(ESGI_RIGHT_BUTTON, ESGI_DOWN, (int)point.x, __height - (int)point.y);
}

- (void) rightMouseUp:(NSEvent *)event
{
    NSPoint point = [self convertPoint:[event locationInWindow] fromView:nil];
    esgiMouseFunc(ESGI_RIGHT_BUTTON, ESGI_UP, (int)point.x, __height - (int)point.y);
}

- (void) otherMouseDown:(NSEvent *)event
{
    NSPoint point = [self convertPoint:[event locationInWindow] fromView:nil];
    esgiMouseFunc(ESGI_MIDDLE_BUTTON, ESGI_DOWN, (int)point.x, __height - (int)point.y);
}

- (void) otherMouseUp:(NSEvent *)event
{
    NSPoint point = [self convertPoint:[event locationInWindow] fromView:nil];
    esgiMouseFunc(ESGI_MIDDLE_BUTTON, ESGI_UP, (int)point.x, __height - (int)point.y);
}

- (void) mouseMoved:(NSEvent *)event
{
    NSPoint point = [self convertPoint:[event locationInWindow] fromView:nil];
    esgiPassiveMotionFunc((int)point.x, __height - (int)point.y);
}

- (void) mouseDragged:(NSEvent *)event
{
    NSPoint point = [self convertPoint:[event locationInWindow] fromView:nil];
    esgiMotionFunc((int)point.x, __height - (int)point.y);
}
...
{  
    ...
    const EsgiContext& context = EsgiGLApplication::g_ThisInstance->GetContext();
    ...
    NSOpenGLPixelFormatAttribute windowedAttrs[] =
    {
        ...
        NSOpenGLPFAOpenGLProfile, context.GetContextProfile() & ESGI_PROFILE_COMPATIBLE ? NSOpenGLProfileVersionLegacy : NSOpenGLProfileVersion3_2Core,
#endif
        0
    };
